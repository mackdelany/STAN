...
def paed_4_11_months_ews(triage_request: TriageRequest) -> str:
    """
    """
    pews, critical = calculate_paed_4_11_months_ews(triage_request)
    pews_message = gen_pews_4_11_message(triage_request, pews, critical)
    return pews, pews_message

def calculate_paed_4_11_months_ews(triage_request: TriageRequest) -> Tuple[int, bool]:
    """
    """
    rr_ews, rr_critical = paed_4_11_months_rr_ews(triage_request.respiratory_rate)
    resp_ews = paed_4_11_months_resp_ews(triage_request.airway_altered, triage_request.breathing_altered)
    sats_ews = paed_4_11_months_sats_ews(triage_request.sats)
    pulse_ews, pulse_critical = paed_4_11_months_pulse_ews(triage_request.vital_signs_pulse)
    bp_sys_ews, bp_sys_critical = paed_4_11_months_bp_sys_ews(triage_request.blood_pressure_systolic)
    loc_ews, loc_critical = paed_4_11_months_loc_ews(triage_request.disability_gcs)
...
def gen_pews_4_11_message(triage_request: TriageRequest, pews: int, critical: bool) -> str:
    """
    """
    if critical:
...
def paed_4_11_months_rr_ews(respiratory_rate: int) -> Tuple[int, bool]:
    """
    """
    rr_ews = 0
    rr_critical = False
    if respiratory_rate:
        if 20 <= respiratory_rate < 45:
            rr_ews = 0
        elif 45 <= respiratory_rate < 50:
            rr_ews = 1
        elif 50 <= respiratory_rate < 55:
            rr_ews = 2
        elif 15 <= respiratory_rate < 20:
            rr_ews = 2
        elif 55 <= respiratory_rate:
            rr_ews = 3
...
def paed_4_11_months_resp_ews(airway_altered: str, breathing_altered: str) -> int:
    """
    """
    resp_ews = 0
    if airway_altered:
...
def paed_4_11_months_sats_ews(sats: float) -> int:
    """
    """
    sats_ews = 0
    if sats:
...
def paed_4_11_months_pulse_ews(vital_signs_pulse: int) -> Tuple[int, bool]:
    """
    """
    pulse_ews = 0
...
def paed_4_11_months_bp_sys_ews(bp_sys: int) -> Tuple[int, bool]:
    """
    """
    bp_sys_ews = 0
...
def paed_4_11_months_loc_ews(disability_gcs: str) -> int:
    """
    """
    loc_ews = 0
    loc_critical = False
